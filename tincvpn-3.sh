...
do
  case "${option}"
    ...
    r) reset="yes" ;;
    u) uninstall="yes" ;;
    *) echo "-i <last_ip_part 192.168.3.?> -p <vpn port if not 655> -c <vpn host to connect to, eg. prx_b> -a <public ip address, or will auto-detect> -r (reset) -u (uninstall)" ; exit ;;
  esac
done
...
echo "Options:"
echo "VPN IP: 192.168.3.${vpn_ip_last}"
...
ip addr add  192.168.3.${vpn_ip_last}/24 dev \$INTERFACE
...
#!/usr/bin/env bash
ip route del 192.168.3.0/24 dev \$INTERFACE
ip addr del 192.168.3.${vpn_ip_last}/24 dev \$INTERFACE
